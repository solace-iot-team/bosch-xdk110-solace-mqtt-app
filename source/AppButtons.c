...
static void appButtons_SetPubTopic(const AppRuntimeConfig_TopicConfig_T * const topicConfigPtr) {
	...
	appButtons_MqttPublishTopicStr = AppMisc_FormatTopic("%s/iot-event/%s/%s/button", topicConfigPtr->received.methodCreate, topicConfigPtr->received.baseTopic, appButtons_DeviceId);
